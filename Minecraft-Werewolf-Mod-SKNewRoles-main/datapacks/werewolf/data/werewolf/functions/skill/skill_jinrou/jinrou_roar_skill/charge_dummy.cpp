...
int main() {
    ...
    // チャージ音と効果
    if (team == "Ponkotsu" && scores["charge_roar"] == 1) {
        playSound("entity.player.breath", player, "~ ~ ~", 1.0f, 1.0f, 0.5f);
        giveEffect(player, "slowness", 1, 3, true);
    }

    // スキル発動
    if (team == "Ponkotsu" && scores["charge_roar"] == 20) {
        executeFunction("werewolf:skill/skill_jinrou/jinrou_roar_skill/main_dummy");
    }

    // チャージ用のスコアボードをリセット
    if (team == "Ponkotsu" && scores["charge_roar"] >= 30) {
        resetScore(player, "charge_roar");
    }

    // 再帰
    if (team == "Ponkotsu" && scores["charge_roar"] >= 0) {
        scheduleFunction("werewolf:skill/skill_jinrou/jinrou_roar_skill/charge_dummy", "1t");
    }
